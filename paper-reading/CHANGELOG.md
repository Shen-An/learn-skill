...
- **本机门禁现状**：`run_evals.py` 的非 PDF 断言全部 PASS，但 `pdf-cases` 用例在 DSH 沙箱（workspace-write）里因往临时目录写 fixture 被拒而中断（先写系统 `%TEMP%`、再改写到工作区内目录，两次都是 `PermissionError`），所以沙箱内跑不到用例 5 / 6 尾部的断言；在沙箱外跑一次即可跑完
