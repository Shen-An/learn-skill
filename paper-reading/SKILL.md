...
- **交付纪律**：
  1. **默认集全部静默落盘，正文只展开用户点名的模式**：说"读论文/讲论文/总结/精读"时 A+B+C+E+F 五件都要落盘（默认集见"选择规则"），正文只展开被点名的那个模式，其余不搬正文；
