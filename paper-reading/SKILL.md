---
name: paper-reading
description: 把学术论文（PDF/摘要/DOI/二手转述）加工成可核查的中文精读笔记：支持 PDF 导入与页码锚点，先一段话总结核心内容，再按"背景→方法→实验→贡献→局限"重建论证链，公式统一用 Markdown 数学语法，并附符号表与来源分层；产出七种互相独立的文件——精读讲解、逐篇填表、思维导图（文本大纲 + 可渲染 Mermaid 两种形态）、难点与解答、多篇汇总综述，以及把整个笔记库管成 wiki 的合集层索引（主题清单 + 术语速查 + 跨论文对比 + 未解决问题 + 证据强度总览）。当用户说"讲一下这篇论文""论文精读""导入这篇 PDF""填这个表格""生成思维导图""这篇论文的难点""可能遇到什么问题""写文献综述""整理我的论文笔记库""更新笔记库索引"或输入 /paper-reading 时使用。
license: MIT
metadata:
  version: "1.4.0"
  author: "Shen-An"
...
2. **一种产出只讲一件事。** 精读讲解、逐篇填表、思维导图、难点与解答、汇总综述各自独立成文件、独立呈现；导图不得内嵌进讲解文档，否则读者要在一篇长文里再看一遍同样的内容。
...
└── <短名>/
    ├── README.md              # 单篇索引：论文清单 + 一句话结论 + 证据强度提示
...
选择规则：用户点名就照办；说"讲论文"默认 A；**C 与 E 成套产出**（大纲是主形态，Mermaid 是渲染形态）；一次给多篇且要求"对比/综述/趋势"走 D（D 不替代 A）；F 可单独要，也可在 A 之后追问；**G 不是可选项**——只要往 `paper-notes/` 里落了新笔记，就按 Step 5 登记进库索引。不确定就问一句，不要默认全套都出。
...
3. 建立来源分层台账（`references/grounding-rules.md`），每条证据打标：`[原文 p.N]` / `[代码]` / `[摘要]` / `[二手]` / `[OCR]` / `[推断]`。
...
### Step 5 · 登记进笔记库索引（模式 G）

落盘新笔记后维护 `paper-notes/README.md`（模板 `references/index-template.md`，机械校验 `--mode index`）：
...
- 先跑机械校验：`python scripts/check_paper_note.py <文件> --mode <auto|deep|table|mindmap|mmd|faq|review|index>`（修完所有 ERROR；WARN 逐条人工判断）
- 再逐条过 `references/quality-rubric.md`（忠实性、论证链完整度、讲解密度这些机器查不了）
- **交付纪律**：
  1. 默认只呈现用户点名的那一个模式的正文；
  2. 用户要多件套时，先给"文件清单 + 每件一句话"，再按文件分别呈现，每块以文件名开头，用分隔线隔开；
  3. **思维导图、填表、难点文档不得与讲解正文混排**——各自独立成块（或独立一条消息）；
...
1. **来源分层与冲突并列**：任何事实断言都要能归到 `[原文 p.N]`/`[代码]`/`[摘要]`/`[二手]`/`[OCR]`/`[推断]`；二手数字必须标"待核对"；两个来源数字不一致时两个都写，不许取平均、不许沉默。
