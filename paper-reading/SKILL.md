---
name: paper-reading
description: 把学术论文（PDF/摘要/DOI/二手转述）加工成可核查的中文精读笔记：支持 PDF 导入与页码锚点，先一段话总结核心内容，再按"背景→方法→实验→贡献→局限"重建论证链，公式统一用 Markdown 数学语法，并附符号表与来源分层；产出六种互相独立的文件——精读讲解、逐篇填表、思维导图（文本大纲 + 可渲染 Mermaid 两种形态）、难点与解答、多篇汇总综述。当用户说"讲一下这篇论文""论文精读""导入这篇 PDF""填这个表格""生成思维导图""这篇论文的难点""可能遇到什么问题""写文献综述"或输入 /paper-reading 时使用。
license: MIT
metadata:
  version: "1.3.0"
  author: "Shen-An"
...
```
paper-notes/<短名>/
├── README.md              # 索引：论文清单 + 一句话结论 + 证据强度提示
...
选择规则：用户点名就照办；说"讲论文"默认 A；**C 与 E 成套产出**（大纲是主形态，Mermaid 是渲染形态）；一次给多篇且要求"对比/综述/趋势"走 D（D 不替代 A）；F 可单独要，也可在 A 之后追问。不确定就问一句，不要默认全套都出。
...
- 先跑机械校验：`python scripts/check_paper_note.py <文件> --mode <auto|deep|table|mindmap|mmd|faq|review>`（修完所有 ERROR；WARN 逐条人工判断）
...
10. **生成期不动 skill**：Step 1–5 期间绝不修改本 skill 自身文件。
